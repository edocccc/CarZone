...
public class RicercaPotenza implements RicercaStrategy {
    ...
    //fornisce quindi la logica per la ricerca di veicoli in base alla potenza

    //dichiarazione del service per la gestione dei veicoli
    private final VeicoloService veicoloService;

    //metodo per la ricerca di veicoli in base alla potenza e alla richiesta ricevuta
    @Override
    public List<Veicolo> ricerca(RicercaRequestDTO request) {
        //dichiarazione di una lista di veicoli
        List<Veicolo> veicoli;
        //estrazione della potenza minima e massima cercata dalla richiesta
        Integer potenzaMinimaCercata = request.getPotenzaMinima();
        Integer potenzaMassimaCercata = request.getPotenzaMassima();
        //invocazione del metodo findByPotenza del service per ottenere i veicoli con la potenza nel range cercato
